...
void NVM_BMWrite(void){
	...
	SSTF016B_Erase( ADDR_BM_IN_NVM/SEC_SIZE , ( ADDR_BM_IN_NVM + sizeof(bmInfo) )/SEC_SIZE  );
	df_write_open(ADDR_BM_IN_NVM);
//	df_write((UNS8 *)&bmInfo,256);
	df_write((UNS8 *)&bmInfo,sizeof(bmInfo));
	df_read_open(ADDR_BM_IN_NVM);    /* set read point*/ 
	df_read((uint8_t *)&bmInfo, sizeof(bmInfo)); //read 256 bytes and set to ReadBuffer?????
}
void NVM_SysinfoWrite(void){
